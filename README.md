...
El contexto académico se construye en una capa central independiente de Ollama. La pregunta guía una selección compacta: una consulta sobre qué estudiar hoy prioriza tareas, Bosses, horario/disponibilidad, eventos del día local y sesiones/estadísticas recientes; una duda sobre una asignatura prioriza esa asignatura, sus temas y los metadatos de materiales relacionados; y una consulta de rendimiento prioriza notas, sesiones y estadísticas. El selector permite añadir elementos concretos y el usuario puede enviar cualquier mensaje sin contexto personal. Se aplican límites por categoría y de caracteres, sin enviar toda la base de datos. Los materiales solo se leen desde almacenamiento y se extraen cuando se pide analizar su contenido o se seleccionan explícitamente; una consulta de metadatos no descarga el archivo. Las imágenes se incluyen únicamente cuando el modelo activo declara soporte de visión. No hay embeddings ni búsqueda semántica.
