...
export function ContextPicker({ enabled, options, value, uploading = false, onChange, onUpload }: { enabled: boolean; options: ContextOptions; value: ContextSelection; uploading?: boolean; onChange: (value: ContextSelection) => void; onUpload?: (files: FileList) => void }) {
  ...
  function toggle(key: keyof ContextSelection, id: string) {
    ...
    const next = current.includes(id) ? current.filter((item) => item !== id) : current.length < 20 ? [...current, id] : current;
    onChange({ ...value, [key]: next });
  }
  return <details className="relative">
    <summary className="flex cursor-pointer list-none items-center gap-2 rounded-lg border bg-card px-3 py-2 text-xs font-medium aria-disabled:cursor-not-allowed aria-disabled:opacity-50" aria-disabled={!enabled}><Paperclip className="size-4" />Contexto{total > 0 && <span className="rounded-full bg-primary px-1.5 py-0.5 text-[10px] text-primary-foreground">{total}</span>}<ChevronDown className="ml-auto size-3.5" /></summary>
