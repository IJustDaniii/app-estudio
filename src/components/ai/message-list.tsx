...
function ContextSummary({ snapshot }: { snapshot?: AIContextSnapshot | null }) {
  ...
  return <div className="mt-3 rounded-lg border bg-muted/40 px-3 py-2 text-xs"><p className="font-medium">{snapshot.mode === "none" || !used ? "Sin contexto personal" : `Contexto usado: ${used}`}</p>{snapshot.blocked.length > 0 && <p className="mt-2 text-muted-foreground">Permiso desactivado: {snapshot.blocked.map((item) => item.label).join(", ")}.</p>}{snapshot.omitted.length > 0 && <p className="mt-1 text-muted-foreground">{snapshot.omitted.length} elemento(s) omitido(s) por el límite configurado.</p>}{snapshot.warnings.length > 0 && <p className="mt-1 text-muted-foreground">Avisos: {snapshot.warnings.join(" ")}</p>}</div>;
}
