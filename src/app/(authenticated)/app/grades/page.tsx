...
export default async function GradesPage() {
  ...
  const summaries = subjects.map((subject) => ({
    id: subject.id,
    name: subject.name,
    weightedSum: subject.grades.reduce(
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-5 py-6 sm:px-8 lg:px-10 lg:py-9">
      <PageHeader
        ...
        description="Registra tus calificaciones, sus pesos y cómo evoluciona tu media por asignatura."
      />
      {subjects.length ? (
        <CreatePanel label="Añadir nota">
          <form
            action={createGrade}
            className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6"
          >
            <div className="space-y-1.5 lg:col-span-2">
              <Label htmlFor="grade-label">Concepto</Label>
              <Input
                id="grade-label"
                name="label"
                required
                maxLength={100}
                placeholder="Examen, práctica…"
              />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="grade-subject">Asignatura</Label>
        ...
        <EmptyState
          title="Crea una asignatura antes de añadir notas"
          description="Las calificaciones siempre pertenecen a una asignatura."
        />
      )}
      <p className="text-sm text-muted-foreground">
        Los días de las notas se interpretan en tu zona horaria:{" "}
        <span className="font-medium text-foreground">{timeZone}</span>.
      </p>
      <GradeCalculator subjects={summaries} />
      {subjects.some((subject) => subject.grades.length) ? (
              ...
              <Card key={subject.id}>
                ...
                <CardContent>
                  <p className="mb-3 text-xs text-muted-foreground">
                    Media ponderada actual · evolución acumulada
                  </p>
                      ...
                      return (
                        <li key={grade.id} className="p-3">
                          <div className="flex items-start gap-3">
                            <div className="grid size-10 shrink-0 place-items-center rounded-lg bg-muted text-sm font-semibold">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="mt-0.5 text-xs text-muted-foreground">
                                {formatDateOnly(grade.date)} · Peso {grade.weight} ·
                                Media tras esta nota:{" "}
                            ...
                            <form action={deleteGrade}>
                              ...
                              <ConfirmSubmit
                                message={`¿Eliminar la nota «${grade.label}»?`}
                                ariaLabel={`Eliminar ${grade.label}`}
                              >
        ...
        <EmptyState
          title="No hay notas"
          description="Añade la primera calificación cuando la tengas."
        />
      )}
    </div>
  );
