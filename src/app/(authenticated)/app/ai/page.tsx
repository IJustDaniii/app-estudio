...
export default async function AIPage({ searchParams }: { searchParams: Promise<{ chat?: string }> }) {
  ...
  const [chatRows, chatTotal, settings, contextOptions] = await Promise.all([
  ...
  const [messageRowsDesc, messageTotal] = activeId ? await Promise.all([
    prisma.aIMessage.findMany({ where: { userId, chatId: activeId }, select: { id: true, role: true, content: true, status: true, model: true, errorCode: true, createdAt: true }, orderBy: { createdAt: "desc" }, take: 50 }),
  ...
  return <AIWorkspace initialChats={chats} initialChatPagination={chatPagination} initialActiveId={activeId} initialMessages={messages} initialMessagePagination={messagePagination} initialSettings={{ ollamaUrl: settings.ollamaUrl, model: settings.model, isAcademicContextEnabled: settings.isAcademicContextEnabled, contextLimit: settings.contextLimit }} contextOptions={contextOptions} />;
}
