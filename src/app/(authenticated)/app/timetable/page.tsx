...
export default async function TimetablePage() {
  ...
  const [subjects, entries, changes] = await Promise.all([
    ...
    prisma.timetableChange.findMany({
      where: { userId },
  ...
  const entryOptions = entries.map((entry) => ({
    id: entry.id,
    label: `${entry.subject.name} · ${days[entry.dayOfWeek - 1]} ${entry.startTime}`,
  }));
    ...
    <div className="mx-auto max-w-[1500px] space-y-6 px-5 py-6 sm:px-8 lg:px-10 lg:py-9">
      ...
      {subjects.length ? (
        <>
          ...
          <CreatePanel label="Añadir cambio puntual">
            <form
              action={createTimetableChange}
              className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6"
            >
              <div className="space-y-1.5">
                <Label htmlFor="change-create-date">Fecha</Label>
                <Input
                  id="change-create-date"
                  name="date"
                  type="date"
                  defaultValue={todayInputValue(timeZone)}
                  required
                />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="change-create-subject">Asignatura</Label>
                <Select id="change-create-subject" name="subjectId" required>
                  {subjects.map((subject) => (
                    <option key={subject.id} value={subject.id}>
                      {subject.name}
                    </option>
              ...
              <div className="space-y-1.5 lg:col-span-2">
                <Label htmlFor="change-create-base">
                  Clase habitual afectada
                </Label>
                <Select id="change-create-base" name="baseEntryId">
                  <option value="">Cambio independiente</option>
              ...
              <label className="flex items-center gap-2 text-sm lg:col-span-6">
                <input type="checkbox" name="isCancelled" value="true" />{" "}
                Cancelar la clase en esa fecha
              </label>
            </form>
          </CreatePanel>
        </>
        ...
        <EmptyState
          title="Crea una asignatura antes de configurar el horario"
          description="Las clases y sus cambios necesitan una asignatura propia."
        />
      )}
                    ...
                    >
                      <div className="flex justify-between gap-2">
                        ...
                        <form action={deleteTimetableEntry}>
                          ...
                          <ConfirmSubmit
                            message={`¿Eliminar la clase habitual de ${entry.subject.name}?`}
                            ariaLabel={`Eliminar clase de ${entry.subject.name}`}
                          >
                      ...
                      <details className="mt-3 border-t pt-2">
                        ...
                        >
                          ...
                          >
                            {days.map((weekday, dayIndex) => (
                              <option key={weekday} value={dayIndex + 1}>
                                {weekday}
                              </option>
                            ))}
                          </Select>
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Solo afectan a la fecha que indiques.
          </p>
        </div>
        {changes.length ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wider text-primary">
                        ...
                        {change.isCancelled ? "Cancelada" : "Cambio puntual"}
                      </p>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {change.isCancelled
                          ? "No hay clase"
                          : `${change.startTime}–${change.endTime}${change.room ? ` · ${change.room}` : ""}`}
                      </p>
                    </div>
                    <form action={deleteTimetableChange}>
                      <input type="hidden" name="id" value={change.id} />
                      <ConfirmSubmit
                        message={`¿Eliminar el cambio del ${formatDateOnly(change.date)}?`}
                        ariaLabel={`Eliminar cambio del ${formatDateOnly(change.date)}`}
                      >
                        <Trash2 className="size-4" />
                      </ConfirmSubmit>
                    </form>
                  </div>
                  <TimetableChangeEditor
          ...
          <p className="rounded-xl border border-dashed p-5 text-sm text-muted-foreground">
            No hay cambios puntuales configurados.
          </p>
        )}
      </section>
    </div>
