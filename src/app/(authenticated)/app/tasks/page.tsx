...
export default async function TasksPage({ searchParams }: { searchParams: Promise<{ error?: string; subjectId?: string }> }) {
  ...
  const [subjects, tasks] = await Promise.all([
    ...
    prisma.task.findMany({ where: { userId }, orderBy: [{ status: "asc" }, { dueDate: "asc" }, { createdAt: "desc" }], include: { subject: true, _count: { select: { materials: true } } } }),
  ]);
  const error = (await searchParams).error;
  const errorMessage = error === "overdue-fixed-deadline" ? "Esta obligación fija ya está vencida y su fecha no se puede reprogramar." : error === "fixed-deadline-date" ? "Una obligación fija necesita una fecha límite." : error === "completed-task" ? "Una tarea completada no se puede volver a abrir desde este editor." : null;
