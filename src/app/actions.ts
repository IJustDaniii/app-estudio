...
import {
  applyAcademicPetProgress,
  hatchEggForUser,
  purchaseCosmeticForUser,
  purchaseEggForUser,
  setActivePetForUser,
...
export type StudyStartResult =
  | { ok: true; requestId: string; startedAt: string; startToken: string }
...
async function currentUserTimeZone(userId: string) {
...
function formDateTime(value: unknown, timeZone: string) {
  const text = String(value ?? "").trim();
  return text ? parseLocalDateTime(text, timeZone) : null;
}

function formDateOnly(value: unknown) {
  return parseDateOnly(String(value ?? ""));
}

async function blockMaterialSubjectChange(
  userId: string,
  field: "topicId" | "taskId" | "bossId",
  id: string,
  ...
  try {
    await ensureAcademicEntitySubjectChangeAllowed(
      userId,
      field,
      id,
      currentSubjectId,
    ...
    if (
      ...
      error.message === "MATERIALS_SUBJECT_CHANGE_BLOCKED"
    )
      redirect(`${path}?error=materials-subject-change`);
    throw error;
  }
}
...
const studySessionMessages: Record<StudySessionErrorCode, string> = {
  ...
  START_TOKEN_PLAN_MISMATCH:
    "La duración de la sesión ha cambiado; vuelve a empezar.",
  ...
  STUDY_MINUTES_EXCEED_ELAPSED:
    "Los minutos indicados superan el tiempo medido por el servidor.",
  STUDY_MINUTES_EXCEED_PLAN:
    "Los minutos indicados superan la duración planificada.",
};

function studySessionError(error: unknown): StudySessionActionState {
  const code =
    error instanceof StudySessionError ? error.code : "INVALID_START_TOKEN";
...
export async function registerAction(
  ...
  if (!parsed.success)
    return {
      error:
        "Revisa los datos. La contraseña necesita 8 caracteres, letra, número y símbolo.",
    };
  ...
  await prisma.user.create({
    data: {
      ...
      subjects: {
        create: DEMO_SUBJECTS.map(([name, color]) => ({ name, color })),
      },
    },
  });
  await signIn("credentials", {
...
export async function updateTopic(formData: FormData) {
  ...
  const data = topicSchema.parse(formObject(formData));
  await ensureOwnedSubject(userId, data.subjectId);
  await blockMaterialSubjectChange(
    userId,
    "topicId",
    id,
    existing.subjectId,
...
export async function createTimetableChange(formData: FormData) {
  ...
  const data = timetableChangeSchema.parse({
    ...raw,
    date: formDateOnly(raw.date),
  });
  const { id: _id, ...changeData } = data;
  void _id;
  await ensureOwnedSubject(userId, changeData.subjectId);
  if (changeData.baseEntryId) {
    const baseEntry = await prisma.timetableEntry.findFirst({
      where: { id: changeData.baseEntryId, userId },
      select: { id: true, subjectId: true },
    });
    if (!baseEntry || baseEntry.subjectId !== changeData.subjectId)
...
export async function updateTimetableChange(formData: FormData) {
  ...
  await prisma.timetableChange.updateMany({
    where: { id, userId },
    data: changeData,
  });
  revalidatePath("/app/timetable");
...
export async function createTask(formData: FormData) {
  const userId = await requireUserId();
  const raw = formObject(formData);
  const timeZone = await currentUserTimeZone(userId);
  const data = taskSchema.parse({
    ...raw,
    dueDate: formDateTime(raw.dueDate, timeZone),
  });
...
export async function updateTask(formData: FormData) {
  ...
  const schedule = canRescheduleTask(
    existing,
    data.dueDate,
    new Date(),
    data.planningMode,
  ...
  if (!schedule.allowed) {
    redirect(
      `/app/tasks?error=${schedule.reason === "OVERDUE_FIXED_DEADLINE" ? "overdue-fixed-deadline" : "fixed-deadline-date"}`,
    );
  }
  if (existing.status === "COMPLETED" && data.status !== "COMPLETED")
    redirect("/app/tasks?error=completed-task");
  const subjectId = await ensureOwnedSubject(userId, data.subjectId);
  await blockMaterialSubjectChange(
    userId,
    "taskId",
  ...
  const completedAt =
    existing.completedAt ?? (data.status === "COMPLETED" ? new Date() : null);
      ...
      prisma.$transaction(
        async (tx) => {
          const updated = await tx.task.updateMany({
            ...
            data: { ...data, subjectId, completedAt },
          });
...
export async function completeTask(formData: FormData) {
  ...
  const id = String(formData.get("id") ?? "");
  const reward = taskCompletionReward();
  const completed = await withSerializableRetry(() =>
    prisma.$transaction(
      async (tx) => {
...
export async function createBoss(formData: FormData) {
  ...
  const raw = formObject(formData);
  const timeZone = await currentUserTimeZone(userId);
  const data = bossSchema.parse({
    ...raw,
    date: formDateTime(raw.date, timeZone),
  });
  await ensureOwnedSubject(userId, data.subjectId);
...
export async function createGoal(formData: FormData) {
  ...
  const data = goalSchema.parse({
    ...raw,
    targetDate: raw.targetDate ? formDateOnly(raw.targetDate) : null,
  });
  const subjectId = await ensureOwnedSubject(userId, data.subjectId);
  await prisma.goal.create({
    data: { ...data, subjectId, isComplete: data.progress === 100, userId },
  });
...
export async function updateGoalDetails(formData: FormData) {
  ...
  await prisma.goal.updateMany({
    where: { id, userId },
    data: { ...data, subjectId, isComplete: data.progress === 100 },
  });
  revalidatePath("/app/goals");
...
export async function updateGoal(formData: FormData) {
  ...
  await prisma.goal.updateMany({
    where: { id, userId },
    data: { progress, isComplete: progress === 100 },
  });
  revalidatePath("/app/goals");
}
...
export async function startStudySession(
  ...
  if (!parsed.success)
    return { ok: false, error: "La duración de la sesión no es válida." };
  try {
    ...
    return {
      ...
      startToken: createStudyStartToken(
        {
          userId,
          requestId,
          startedAt,
    ...
    if (error instanceof StudySessionError)
      return { ok: false, error: studySessionMessages[error.code] };
    throw error;
  }
}

export async function recordStudySession(
  _state: StudySessionActionState,
  formData: FormData,
  ...
  if (!parsed.success)
    return {
      error: "No se pudo validar la sesión. Vuelve a iniciar el temporizador.",
    };
  const data = parsed.data;
  let sessionData;
  try {
    const start = verifyStudyStartToken(data.startToken, studyTokenSecret(), {
      userId,
      requestId: data.requestId,
      plannedMinutes: data.plannedMinutes,
    });
    sessionData = validateServerStudySession(
      {
  ...
  try {
    const petProgress = await withSerializableRetry(() =>
      prisma.$transaction(
        async (tx) => {
    ...
    if (
      !existing ||
      !sameStudySessionRequest(existing, { ...sessionData, subjectId, taskId })
    ) {
    ...
    return {
      success:
        "La sesión ya estaba guardada; no se han repetido las recompensas.",
    };
  }
}
      ...
      (
        {
          ...
          INVALID_REQUEST_ID:
            "No se pudo validar la operación. Inténtalo de nuevo.",
        } as Record<string, string>
      )[code] ?? "No se pudo completar la operación.",
  };
}

export async function purchaseEggAction(
  _state: PetActionState,
  formData: FormData,
): Promise<PetActionState> {
  const userId = await requireUserId();
  try {
    const data = petPurchaseSchema.parse(
    ...
    return {
      success: result.duplicate
        ? "La compra ya estaba aplicada."
        : "Huevo añadido al inventario.",
    };
  } catch (error) {
...
export async function startEggIncubationAction(
  _state: PetActionState,
  formData: FormData,
): Promise<PetActionState> {
  const userId = await requireUserId();
  try {
    const { eggId } = eggIdActionSchema.parse(
      Object.fromEntries(formData.entries()),
    );
...
export async function hatchEggAction(
  ...
  try {
    ...
    return {
      success: result.duplicate
        ? "Duplicado convertido en " + result.fragmentQuantity + " fragmento."
        : result.speciesName +
          " ha eclosionado · " +
          PET_RARITY_CONFIG[result.rarity].label +
          ".",
    };
  } catch (error) {
    return petActionError(error);
...
export async function setActivePetAction(
  ...
  const userId = await requireUserId();
  try {
    const { petId } = petIdActionSchema.parse(
      Object.fromEntries(formData.entries()),
    );
...
export async function purchaseCosmeticAction(
  ...
  const userId = await requireUserId();
  try {
    const data = cosmeticPurchaseSchema.parse(
      Object.fromEntries(formData.entries()),
    );
    const result = await purchaseCosmeticForUser(
      userId,
      data.cosmeticSlug,
      data.requestId,
    );
    ...
    return {
      success: result.duplicate
        ? "La compra ya estaba aplicada."
        : "Cosmético añadido al inventario.",
    };
  } catch (error) {
    return petActionError(error);
  }
